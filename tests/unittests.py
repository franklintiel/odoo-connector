...
class TestOdooInstance(unittest.TestCase):
    ...
    def test_calling_create_method(self):
        ...
        if DummieData.CAN_CREATE:
            uid = self.odoo.connect()
            result = self.odoo.write(uid=uid, model='res.partner', action='create', data=DummieData.NEW_PARTNER_DATA)
    ...
    def test_calling_update_method(self):
        ...
        if DummieData.CAN_UPDATE and DummieData.ID_TO_UPDATE > 0:
            ...
            uid = self.odoo.connect()
            result = self.odoo.write(uid=uid, model='res.partner', action='write', data=data)
            self.assertIsNotNone(result)
            self.assertGreater(result, 0)
            self.assertGreater(uid, 0)
        else:
    ...
    def test_calling_unlink_method(self):
        ...
        if DummieData.CAN_DELETE and DummieData.ID_TO_DELETE > 0:
            ...
            result = self.odoo.write(uid=uid, model='res.partner', action='unlink', data=data)
            self.assertIsNotNone(result)
            self.assertGreater(result, 0)
            self.assertGreater(uid, 0)
        else:
    ...
    def test_calling_signup_method(self):
        ...
        if DummieData.CAN_SIGNUP:
            ...
            uid = self.odoo.connect()
            result = self.odoo.write(uid=uid, model='res.users', action='signup', data=data)
            self.assertIsNotNone(result)
            self.assertGreater(result, 0)
            self.assertGreater(uid, 0)
        else:
    ...
    def test_calling_search_method(self):
        ...
        result = self.odoo.search(
            uid=uid, model='res.partner', action='search', queries=DummieData.QUERY_FILTER, parameters=None)
        self.assertIsNotNone(result)
    ...
    def test_calling_search_with_pagination_method(self):
        ...
        uid = self.odoo.connect()
        result = self.odoo.search(
            uid=uid, model='res.partner', action='search', queries=DummieData.QUERY_FILTER,
    ...
    def test_calling_search_count_method(self):
        ...
        uid = self.odoo.connect()
        result = self.odoo.search(
            uid=uid, model='res.partner', action='search_count', queries=DummieData.QUERY_FILTER)
        self.assertIsNotNone(result)
        self.assertGreaterEqual(uid, 0)
    ...
    def test_calling_read_method(self):
        ...
        if len(ids) > 0:
            result = self.odoo.search(uid=uid, model='res.partner', action='read', queries=ids)
        self.assertIsNotNone(result)
    ...
    def test_calling_search_read_only_fields_method(self):
        ...
        result = self.odoo.search(
            ...
            parameters={'fields': DummieData.PARTNER_FIELDS},
            formatted=True)
        self.assertIsNotNone(result)
        self.assertGreaterEqual(len(result), 0)
        self.assertGreater(uid, 0)
    ...
    def test_any_method(self):
        ...
        if DummieData.ID_TO_SELECT > 0:
            uid = self.odoo.connect()
            result = self.odoo.search(
                uid=uid, model='res.partner', action='name_get', queries=[[DummieData.ID_TO_SELECT]])
            self.assertIsNotNone(result)
            self.assertGreater(result, 0)
            self.assertGreater(uid, 0)
        else:
